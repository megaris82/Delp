...
// Find a user by username (used by login and to check for duplicates).
...
// Create a new user. The password is hashed with bcrypt before saving.
...
// Compare a plaintext password from the login form with the stored hash.
...
// Update a user's editable fields (profile info, role, registration status).
...
// Delete a user. Returns true if a row was actually removed.
async function remove(id) {
  const [result] = await pool.query("DELETE FROM users WHERE id = ?", [id]);
  return result.affectedRows > 0;
}

// Count how many admins exist. Used to stop the last admin from being removed.
